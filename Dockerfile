...
RUN echo "docker exec workbench echo "docker exec workbench sudo curl -s https://raw.githubusercontent.com/rancher/k3d/main/install.sh | bash" >> /nexus-bucket/workbench.sh" >> deploy-olympiad.sh
