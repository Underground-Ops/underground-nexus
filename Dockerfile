...
RUN echo 'wait_docker() { i=0; until docker info >/dev/null 2>&1; do i=$((i+1)); [ "$i" -ge 180 ] && { log "dockerd not ready after 180s - continuing best-effort"; return 1; }; [ $((i % 5)) -eq 0 ] && log "waiting for docker daemon... ($i)"; sleep 1; done; log "docker daemon ready"; }' >> deploy-olympiad.sh
RUN echo 'wait_container() { n="$1"; i=0; until [ "$(docker inspect -f "{{.State.Running}}" "$n" 2>/dev/null)" = "true" ]; do i=$((i+1)); [ "$i" -ge 120 ] && { log "$n not running after 120s - continuing"; return 1; }; sleep 1; done; log "$n running"; }' >> deploy-olympiad.sh
RUN echo 'wait_exec() { n="$1"; wait_container "$n" || true; i=0; until docker exec "$n" true >/dev/null 2>&1; do i=$((i+1)); [ "$i" -ge 120 ] && { log "$n not exec-ready after 240s - continuing"; return 1; }; sleep 2; done; log "$n exec-ready"; }' >> deploy-olympiad.sh
RUN echo 'retry() { n=0; until "$@"; do n=$((n+1)); [ "$n" -ge 5 ] && { log "failed after 5 tries: $*"; return 1; }; log "retry $n/5: $*"; sleep 3; done; }' >> deploy-olympiad.sh
RUN echo 'ensure_net_bridge() { docker network inspect "$1" >/dev/null 2>&1 && { log "net $1 exists"; return 0; }; retry docker network create -d bridge --subnet="$2" --gateway="$3" "$1"; }' >> deploy-olympiad.sh
RUN echo 'ensure_net_overlay() { docker network inspect "$1" >/dev/null 2>&1 && { log "net $1 exists"; return 0; }; retry docker network create -d overlay --subnet="$2" "$1"; }' >> deploy-olympiad.sh
RUN echo 'ensure_repo() { if [ -d /nexus-bucket/underground-nexus/.git ]; then git -C /nexus-bucket/underground-nexus pull --rebase >/dev/null 2>&1 || true; else mkdir -p /nexus-bucket; retry git clone https://github.com/Underground-Ops/underground-nexus.git /nexus-bucket/underground-nexus || true; fi; }' >> deploy-olympiad.sh
RUN echo 'pkg_add() { command -v "$1" >/dev/null 2>&1 && return 0; apk add --no-cache "$2" >/dev/null 2>&1 || { apt-get update >/dev/null 2>&1 && apt-get install -y "$2" >/dev/null 2>&1; } || dnf install -y "$2" >/dev/null 2>&1 || true; command -v "$1" >/dev/null 2>&1; }' >> deploy-olympiad.sh
RUN echo 'ensure_compose() { pkg_add docker-compose docker-compose || log "docker-compose not installable here - compose steps degrade"; }' >> deploy-olympiad.sh
...
RUN echo '' >> deploy-olympiad.sh
RUN echo '# ---- preflight (added): wait for dockerd, ensure repo, clean slate ----' >> deploy-olympiad.sh
RUN echo 'wait_docker' >> deploy-olympiad.sh
RUN echo 'detect_substrate' >> deploy-olympiad.sh
RUN echo 'pkg_add git git || log "git unavailable - repo sync may skip"' >> deploy-olympiad.sh
RUN echo 'ensure_repo' >> deploy-olympiad.sh
RUN echo '[ "${OLYMPIAD_CLEAN:-1}" = "1" ] && for c in Inner-DNS-Control Olympiad0 Athena0 Security-Operation-Center workbench torpedo code-server Nexus-Secret-Vault; do docker rm -f "$c" >/dev/null 2>&1 || true; done' >> deploy-olympiad.sh

#Build Inner-Athena engine
RUN echo 'ensure_swarm || true' >> deploy-olympiad.sh
RUN echo 'ensure_compose' >> deploy-olympiad.sh

RUN echo 'ensure_net_bridge Inner-Athena 10.20.0.0/24 10.20.0.1' >> deploy-olympiad.sh
RUN echo 'docker run -itd -p ${P_DNS}:53/tcp -p ${P_DNS}:53/udp -p ${P_DHCP}:67 -p 800:80 -h Inner-DNS-Control --name=Inner-DNS-Control --net=Inner-Athena --ip=10.20.0.20 --restart=always -v pihole_DNS_data:/etc/dnsmasq.d/ -v pihole_config:/etc/pihole/ pihole/pihole:latest || true' >> deploy-olympiad.sh
RUN echo 'wait_container Inner-DNS-Control' >> deploy-olympiad.sh

#Build Olympiad0 Portainer node
RUN echo "docker volume create portainer_data" >> deploy-olympiad.sh
RUN echo "docker run -d -p 8000:8000 -p 9443:9443 --name=Olympiad0 --dns=10.20.0.20 --net=Inner-Athena --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce:latest || true" >> deploy-olympiad.sh
...
RUN echo 'wait_exec Security-Operation-Center' >> deploy-olympiad.sh
RUN echo "docker exec Security-Operation-Center apt update || true" >> deploy-olympiad.sh
RUN echo "docker exec Security-Operation-Center apt install rofi -y || true" >> deploy-olympiad.sh
RUN echo "docker exec Security-Operation-Center apt install terminator -y || true" >> deploy-olympiad.sh
RUN echo "docker exec Security-Operation-Center apt install firefox -y || true" >> deploy-olympiad.sh
RUN echo "docker exec Security-Operation-Center su - abc -c 'DISPLAY=:1 firefox &' || true" >> deploy-olympiad.sh
RUN echo "docker exec Security-Operation-Center su - abc -c 'DISPLAY=:1 terminator &' || true" >> deploy-olympiad.sh
...
RUN echo 'wait_exec workbench' >> deploy-olympiad.sh
...
RUN echo 'log() { echo "[olympiad] $*"; }' >> deploy-olympiad-light.sh
RUN echo 'wait_docker() { i=0; until docker info >/dev/null 2>&1; do i=$((i+1)); [ "$i" -ge 180 ] && { log "dockerd not ready after 180s - continuing best-effort"; return 1; }; [ $((i % 5)) -eq 0 ] && log "waiting for docker daemon... ($i)"; sleep 1; done; log "docker daemon ready"; }' >> deploy-olympiad-light.sh
RUN echo 'wait_container() { n="$1"; i=0; until [ "$(docker inspect -f "{{.State.Running}}" "$n" 2>/dev/null)" = "true" ]; do i=$((i+1)); [ "$i" -ge 120 ] && { log "$n not running after 120s - continuing"; return 1; }; sleep 1; done; log "$n running"; }' >> deploy-olympiad-light.sh
RUN echo 'wait_exec() { n="$1"; wait_container "$n" || true; i=0; until docker exec "$n" true >/dev/null 2>&1; do i=$((i+1)); [ "$i" -ge 120 ] && { log "$n not exec-ready after 240s - continuing"; return 1; }; sleep 2; done; log "$n exec-ready"; }' >> deploy-olympiad-light.sh
RUN echo 'retry() { n=0; until "$@"; do n=$((n+1)); [ "$n" -ge 5 ] && { log "failed after 5 tries: $*"; return 1; }; log "retry $n/5: $*"; sleep 3; done; }' >> deploy-olympiad-light.sh
RUN echo 'ensure_net_bridge() { docker network inspect "$1" >/dev/null 2>&1 && { log "net $1 exists"; return 0; }; retry docker network create -d bridge --subnet="$2" --gateway="$3" "$1"; }' >> deploy-olympiad-light.sh
RUN echo 'ensure_net_overlay() { docker network inspect "$1" >/dev/null 2>&1 && { log "net $1 exists"; return 0; }; retry docker network create -d overlay --subnet="$2" "$1"; }' >> deploy-olympiad-light.sh
RUN echo 'ensure_repo() { if [ -d /nexus-bucket/underground-nexus/.git ]; then git -C /nexus-bucket/underground-nexus pull --rebase >/dev/null 2>&1 || true; else mkdir -p /nexus-bucket; retry git clone https://github.com/Underground-Ops/underground-nexus.git /nexus-bucket/underground-nexus || true; fi; }' >> deploy-olympiad-light.sh
RUN echo 'pkg_add() { command -v "$1" >/dev/null 2>&1 && return 0; apk add --no-cache "$2" >/dev/null 2>&1 || { apt-get update >/dev/null 2>&1 && apt-get install -y "$2" >/dev/null 2>&1; } || dnf install -y "$2" >/dev/null 2>&1 || true; command -v "$1" >/dev/null 2>&1; }' >> deploy-olympiad-light.sh
...
RUN echo '' >> deploy-olympiad-light.sh
RUN echo '# ---- preflight: wait for dockerd, ensure repo, clean slate ----' >> deploy-olympiad-light.sh
RUN echo 'wait_docker' >> deploy-olympiad-light.sh
RUN echo 'detect_substrate' >> deploy-olympiad-light.sh
RUN echo 'pkg_add git git || log "git unavailable - repo sync may skip"' >> deploy-olympiad-light.sh
RUN echo 'ensure_repo' >> deploy-olympiad-light.sh
RUN echo '[ "${OLYMPIAD_CLEAN:-1}" = "1" ] && for c in Inner-DNS-Control Olympiad0 workbench Athena0 torpedo; do docker rm -f "$c" >/dev/null 2>&1 || true; done' >> deploy-olympiad-light.sh
RUN echo 'ensure_swarm || true' >> deploy-olympiad-light.sh
RUN echo 'ensure_compose' >> deploy-olympiad-light.sh
RUN echo 'ensure_net_bridge Inner-Athena 10.20.0.0/24 10.20.0.1' >> deploy-olympiad-light.sh
RUN echo "docker run -itd -p 800:80 -h Inner-DNS-Control --name=Inner-DNS-Control --net=Inner-Athena --ip=10.20.0.20 --restart=always -v pihole_DNS_data:/etc/dnsmasq.d/ -v pihole_config:/etc/pihole/ pihole/pihole:latest || true" >> deploy-olympiad-light.sh
RUN echo 'wait_container Inner-DNS-Control' >> deploy-olympiad-light.sh
RUN echo "docker volume create portainer_data || true" >> deploy-olympiad-light.sh
RUN echo "docker run -d -p 8000:8000 -p 9443:9443 --name=Olympiad0 --dns=10.20.0.20 --net=Inner-Athena --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce:latest || true" >> deploy-olympiad-light.sh
...
RUN echo 'wait_exec workbench' >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt -y update >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
...
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y git >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y iputils-ping >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo wget https://github.com/shiftkey/desktop/releases/download/release-3.1.1-linux1/GitHubDesktop-linux-3.1.1-linux1.deb >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo dpkg -i GitHubDesktop-linux-3.1.1-linux1.deb >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y apt-transport-https curl >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench wget https://dl.google.com/linux/direct/chrome-remote-desktop_current_amd64.deb >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo dpkg -i chrome-remote-desktop_current_amd64.deb >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench wget https://release.gitkraken.com/linux/gitkraken-amd64.deb >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo dpkg -i gitkraken-amd64.deb >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
...
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y qemu >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y qemu-kvm libvirt-daemon-system libvirt-clients bridge-utils qemu-system qemu-system-x86 qemu-system-arm >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y virt-manager >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y synaptic >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec Athena0 sh /enable-weekly-updates.sh >> /nexus-bucket/enable-weekly-updates.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo wget https://raw.githubusercontent.com/Underground-Ops/underground-nexus/main/Terraform%20Master/terraform-workbench-install.sh >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo sh terraform-workbench-install.sh >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo mv /terraform-workbench-install.sh /config/Desktop/nexus-bucket >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt -y update --fix-missing >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
...
RUN echo "docker exec workbench echo docker exec workbench sudo apt -y upgrade >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo apt install -y virt-manager >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench sudo rm /usr/share/backgrounds/ubuntu-mate-jammy/Jammy-Jellyfish_WP_4096x2304_Green.png >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
...
RUN echo 'wait_exec Athena0' >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 apt -y update || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 apt install -y iputils-ping || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 git clone https://github.com/radareorg/radare2 || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 sh radare2/sys/install.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 apt-get install -y metasploit-framework || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 apt -y update || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 apt -y upgrade || true" >> deploy-olympiad-light.sh
RUN echo 'docker run -itd --privileged -p ${P_S3}:9000 -p 9010:9001 --name=torpedo -h torpedo --dns=10.20.0.20 --net=Inner-Athena --restart=always -v /nexus-bucket:/nexus-bucket -v /nexus-bucket/s3-torpedo:/data pgsty/minio:latest server /data --console-address :9001 || true' >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 sh /old-underground-nexus-dagger-ci.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 sh /nexus-bucket/underground-nexus/'Dagger CI'/Scripts/enable-weekly-updates.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench echo docker exec workbench bash /config/Desktop/nexus-bucket/underground-nexus/visual-studio-code.sh >> /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec Athena0 sh /nexus-bucket/workbench.sh || true" >> deploy-olympiad-light.sh
RUN echo "docker exec workbench bash /config/Desktop/nexus-bucket/terraform-workbench-install.sh && docker exec workbench terraform -v || true" >> deploy-olympiad-light.sh
...
RUN echo "docker restart Inner-DNS-Control || true" >> deploy-olympiad-light.sh
