...
RUN wget -O /nexus-devsecops-appinator.sh "https://raw.githubusercontent.com/Underground-Ops/underground-nexus/refs/heads/main/Dagger%20CI/Scripts/nexus-devsecops-appinator.sh" && bash /nexus-devsecops-appinator.sh || true
...
bash /nexus-bucket/underground-nexus/update-git-packages.sh || true\n\
exec /bin/bash' > /usr/local/bin/start_services.sh && chmod +x /usr/local/bin/start_services.sh
